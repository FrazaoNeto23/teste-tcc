...
<body>
    <div class="header">
        <div class="header-container">
            <div class="logo">
                ...
                <button onclick="atualizarPedidos(); atualizarStats();" class="btn btn-primary" style="margin-left: 15px; padding: 8px 16px; font-size: 12px;">
                    <i class="fas fa-sync-alt"></i> Atualizar Agora
                </button>
                <span class="auto-update-badge" style="background: #6c757d; margin-left: 10px;">
            ...
            <div style="display: flex; gap: 15px; flex-wrap: wrap;">
                ...
                <a href="limpar_pedidos_antigos.php?executar=1" class="btn btn-warning" onclick="return confirm('Deseja limpar pedidos finalizados (entregues e cancelados)?')">
                    <i class="fas fa-trash-alt"></i> Limpar Finalizados
                </a>
                            ...
                            <div class="pedido-header">
                                <div style="flex:1;">
                                    <?php if ($pedido['conta_solicitada'] == 1): ?>
                                        <div style="background: #ffc107; color: #000; padding: 8px 12px; border-radius: 8px; margin-bottom: 10px; font-weight: bold; display: inline-block;">
                                            <i class="fas fa-receipt"></i> CLIENTE PEDIU A CONTA!
                                        </div>
                                    <?php endif; ?>
                                ...
                                <div class="status-form">
                                    ...
                                    <a href="imprimir_pedido.php?id=<?= $pedido['id'] ?>" target="_blank" 
                                       class="btn btn-primary" style="padding:10px 20px; text-decoration: none; display: inline-flex; align-items: center; gap: 8px;">
                                        <i class="fas fa-print"></i> Conta
                                    </a>
                                </div>
                            </div>
                        </div>
    ...
    <script>
            ...
            try {
                ...
                if (data.pedidos && data.pedidos.length > 0) {
                    ...
                    data.pedidos.forEach(pedido => {
                        ...
                        html += `
                            <div class="pedido ${isNovo ? 'pedido-novo' : ''} ${pedido.conta_solicitada == 1 ? 'pedido-conta-solicitada' : ''}" id="pedido-${pedido.id}">
                                <div class="pedido-header">
                                    <div style="flex:1;">
                                        ${pedido.conta_solicitada == 1 ? `
                                            <div style="background: #ffc107; color: #000; padding: 8px 12px; border-radius: 8px; margin-bottom: 10px; font-weight: bold; display: inline-block;">
                                                <i class="fas fa-receipt"></i> CLIENTE PEDIU A CONTA!
                                            </div>
                                        ` : ''}
        ...
        // Atualizar automaticamente a cada 30 segundos (mudado de 5 para 30)
        setInterval(() => {
            atualizarPedidos();
            atualizarStats();
        }, 30000);
